%% Optimize a Cross-Validated SVM Classifier Using |fminsearch|
...
z = exp(searchmin)
%%
% The surface seems to have many local minima.  Try a 20 random sets of
...
g = nan(7,1);
figure;
g(1:2) = gscatter(cdata(:,1),cdata(:,2),grp,'rg','+*');
hold on
g(3:4) = gscatter(newData(:,1),newData(:,2),v,'mc','**');
g(5) = plot(cdata(SVMModel.IsSupportVector,1),...
    cdata(SVMModel.IsSupportVector,2),'ko');
contour(x1Grid,x2Grid,reshape(scores(:,2),size(x1Grid)),[0 0],'k');
legend(g(1:5),{'-1 (training)','+1 (training)','-1 (classified)',...
...
for ii = mydiff % Plot red circles around correct pts
    g(6) = plot(newData(ii,1),newData(ii,2),'ro','MarkerSize',12);
end

for ii = not(mydiff) % Plot black circles around incorrect pts
    g(7) = plot(newData(ii,1),newData(ii,2),'ko','MarkerSize',12);
end
legend(g,{'-1 (training)','+1 (training)','-1 (classified)',...
