...
% importance_ measure (|OOBPredictorImportance|), and plot the results to visually find
...
% but MATLAB deployment tools could greatly improve the workflow here.
...
%%
% The rows correspond to the actual ratings, and the columns to the
% predicted ratings. The amount in the position |(i,j)| in the confusion
% matrix indicates how many customers received an actual rating |i| and
% were predicted as rating |j|. For example, position |(3,2)| tells us how
% many customers received a rating of 'A' by the credit committee, but were
% predicted as 'AA' with the automated classifier. One can also present
% this matrix in percentage. Normalize each value by the number of
% observations that has the same true rating.
...
% analysis and naive Bayes classifiers. MATLAB also offers Deep Learning
% Toolbox(TM). Also, Database Toolbox and MATLAB deployment tools may
...
% Available at: https://www.bis.org/publ/bcbsca.htm.
%
