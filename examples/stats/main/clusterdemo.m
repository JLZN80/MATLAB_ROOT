...
% The function <docid:stats_ug#buefs04 kmeans> performs K-Means clustering, using an iterative
...
% points from the upper cluster (plotted with squares). Because the upper 
% cluster is so spread out, those three points are closer
% to the centroid of the lower cluster than to that of the upper cluster,
% even though the points are separated from the bulk of the points in their own
...
% name-value pair argument to print out information about each iteration in the clustering
% algorithm.
[cidx3,cmeans3] = kmeans(meas,3,'Display','iter');

%%
% At each iteration, the |kmeans| algorithm 
% (see <docid:stats_ug#bues2hs Algorithms>) reassigns points among clusters
...
% selected set of initial centroid locations. The |kmeans| algorithm can converge to 
% a solution that is a local minimum; that is, |kmeans| can partition the
% data such that moving any single point to a different cluster increases the total sum of
% distances. However, as with many other types of numerical minimizations, 
% the solution that |kmeans| reaches sometimes depends on the starting points.
% Therefore, other solutions (local minima) that have a lower total sum of 
% distances can exist for the data. You can use the optional |'Replicates'| 
% name-value pair argument to test different solutions. When you specify
% more than one replicate, |kmeans| repeats the clustering process starting
% from different randomly selected centroids for each replicate. |kmeans| then returns
% the solution with the lowest total sum of distances among all the
% replicates.
