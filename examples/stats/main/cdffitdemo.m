...
% This example shows how to fit univariate distributions using least
% squares estimates of the cumulative distribution functions.
% This is a generally-applicable method that can be useful in cases when
% maximum likelihood fails, for instance some models that include a
% threshold parameter.
%
% The most common method for fitting a univariate distribution to data is
% maximum likelihood.  But maximum likelihood does not work in all cases,
% and other estimation methods, such as the Method of Moments, are
% sometimes needed.  When applicable, maximum likelihood is probably the
% better choice of methods, because it is often more efficient. But the
% method described here provides another tool that can be used when needed.
...
% The term "least squares" is most commonly used in the context of fitting
% a regression line or surface to model a response variable as a function
% of one or more predictor variables.  The method described here is a very
% different application of least squares: univariate distribution fitting,
% with only a single variable.
%
% To begin, first simulate some sample data.  We'll use an exponential
% distribution to generate the data.  For the purposes of this example, as
% in practice, we'll assume that the data are not known to have come from a
% particular model.
rng(37,'twister');
n = 100;
x = exprnd(2,n,1);
...
%%
% Next, compute the empirical cumulative distribution function (ECDF) of
% the data. This is simply a step function with a jump in cumulative
% probability, p, of 1/n at each data point, x.
x = sort(x);
p = ((1:n)-0.5)' ./ n;
stairs(x,p,'k-');
xlabel('x');
...
%%
% We'll fit an exponential distribution to these data.  One way to do that
% is to find the exponential distribution whose cumulative distribution
% function (CDF) best approximates (in a sense to be explained below) the
% ECDF of the data.  The exponential CDF is p = Pr{X <= x} = 1 -
% exp(-x/mu).  Transforming that to -log(1-p)*mu = x gives a linear
% relationship between -log(1-p) and x.  If the data do come from an
% exponential, we ought to see, at least approximately, a linear
% relationship if we plug the computed x and p values from the ECDF into
% that equation.  If we use least squares to fit a straight line through
% the origin to x vs. -log(1-p), then that fitted line represents the
% exponential distribution that is "closest" to the data.  The slope of the
% line is an estimate of the parameter mu.
%
% Equivalently, we can think of y = -log(1-p) as an "idealized sample" from
% a standard (mean 1) exponential distribution. These idealized values are
...
% distribution, and we'll fit the least squares line through the origin to
% x vs. y.
y = -log(1 - p);
muHat = y \ x
...
ylabel('y = -log(1-p)');
%%
% Notice that the linear fit we've made minimizes the sum of squared errors
% in the horizontal, or "x", direction.  That's because the values for y =
% -log(1-p) are deterministic, and it's the x values that are random.  It's
% also possible to regress y vs. x, or to use other types of linear fits,
% for example, weighted regression, orthogonal regression, or even robust
...
%%
% The two methods give very similar fitted distributions, although the LS
% fit has been influenced more by observations in the tail of the
% distribution.
...
% Weibull is p = Pr{X <= x} = 1 - exp(-(x/a)^b). Transforming that to
% log(a) + log(-log(1-p))*(1/b) = log(x) again gives a linear relationship,
% this time between log(-log(1-p)) and log(x).  We can use least squares to
% fit a straight line on the transformed scale using p and x from the ECDF,
% and the slope and intercept of that line lead to estimates of a and b.
logx = log(x);
logy = log(-log(1 - p));
...
% It's sometimes necessary to fit positive distributions like the Weibull
% or lognormal with a threshold parameter.  For example, a Weibull random
% variable takes values over (0,Inf), and a threshold parameter, c, shifts
% that range to (c,Inf).  If the threshold parameter is known, then there
% is no difficulty.  But if the threshold parameter is not known, it must
% instead be estimated.  These models are difficult to fit with maximum
% likelihood -- the likelihood can have multiple modes, or even become
% infinite for parameter values that are not reasonable for the data, and
% so maximum likelihood is often not a good method.  But with a small
% addition to the least squares procedure, we can get stable estimates.
%
...
%%
% How can we fit a three-parameter Weibull distribution to these data?  If
% we knew what the threshold value was, 1 for example, we could subtract
% that value from the data and then use the least squares procedure to
% estimate the Weibull shape and scale parameters.
x = sort(x);
p = ((1:n)-0.5)' ./ n;
logy = log(-log(1-p));
logxm1 = log(x-1);
...
% threshold value.  If we try subtracting different threshold values, we
% get different plots and different parameter estimates.
logxm2 = log(x-2);
...
%%
% The relationship between log(x-4) and log(-log(1-p)) appears
% approximately linear.  Since we'd expect to see an approximately linear
% plot if we subtracted the true threshold parameter, this is evidence that
% 4 might be a reasonable value for the threshold.  On the other hand, the
% plots for 2 and 3 differ more systematically from linear, which is
% evidence that those values are not consistent with the data.
%
...
% linear regression on the transformed variables log(x-c) and
% log(-log(1-p)). To estimate the threshold parameter, we can carry that
% one step further, and maximize the R^2 value over all possible threshold
% values.
...
%%
% The second set of values for a and b make for a much better plot, and
% thus are more compatible with the data, if you are measuring "compatible"
% by how straight you can make the P-P plot.
%
% To make the scatter match the 1:1 line as closely possible, we can find
% the values of a and b that minimize a weighted sum of the squared
% distances to the 1:1 line.  The weights are defined in terms of the
% empirical probabilities, and are lowest in the center of the plot and
% highest at the extremes.  These weights compensate for the variance of
% the fitted probabilities, which is highest near the median and lowest in
% the tails. This weighted least squares procedure defines the estimator
% for a and b.
...
%%
% Notice that in the location-scale cases considered earlier, we could fit
% the distribution with a single straight line fit.  Here, as with the
% threshold parameter example, we had to iteratively find the best-fit
% parameter values.
...
% The lognormal distribution is simple to fit by maximum likelihood,
% because once the log transformation is applied to the data, maximum
% likelihood is identical to fitting a normal.  But it is sometimes
% necessary to estimate a threshold parameter in a lognormal model.  The
% likelihood for such a model is unbounded, and so maximum likelihood does
% not work.  However, the least squares method provides a way to make
% estimates.  Since the two-parameter lognormal distribution can be
% log-transformed to a location-scale family, we could follow the same
% steps as in the earlier example that showed fitting a Weibull
% distribution with threshold parameter.  Here, however, we'll do the
% estimation on the cumulative probability scale, as in the previous
% example showing a fit with the gamma distribution.
%
...
% With maximum likelihood, the usual method is to use the information
% matrix and a large-sample asymptotic argument to approximate the
% covariance matrix of the estimator over repeated sampling.  No such
% theory exists for these least squares estimators.
%
% However, Monte-Carlo simulation provides another way to estimate standard
% errors.  If we use the fitted model to generate a large number of
% datasets, we can approximate the standard error of the estimators with
% the Monte-Carlo standard deviation.  For simplicity, we've defined a
% fitting function in a separate file, <matlab:edit('logn3fit.m')
% |logn3fit.m|>.
...
%%
% Clearly, the estimator for the threshold parameter is skewed.  This is to
% be expected, since it is bounded above by the minimum data value.  The
% other two histograms indicate that approximate normality might be a
% questionable assumption for the log-location parameter (the first
% histogram) as well.  The standard errors computed above must be
% interpreted with that in mind, and the usual construction for confidence
% intervals might not be appropriate for the log-location and threshold
% parameters.
%
% The means of the simulated estimates are close to the parameter values
% used to generate simulated data, indicating that the procedure is
% approximately unbiased at this sample size, at least for parameter values
% near the estimates.
...
% calculations. That's not surprising, since the fitted model is the same
% one from which the example data were generated.
...
% fails to provide useful parameter estimates.  One important application
% is in fitting distributions involving a threshold parameter, such as the
...
%
% The P-P plots used here to illustrate the fitting method are useful in
% their own right, as a visual indication of lack of fit when fitting a
