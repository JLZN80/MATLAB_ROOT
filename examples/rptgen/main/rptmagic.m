...
while ~strcmp(chapter.CurrentHoleId, '#end#')
    switch chapter.CurrentHoleId
        ...
        case 'MagicSquareTable'
            % If the square's rank is less than 25, assume that it
            % will fit onto a page as a table.
            if rank < 25
        ...
        case 'MagicSquareImage'
            % If the square's rank is greater or equal to 25, assume that a
            % table will not fit, instead render as image.
            if rank >= 25
                % Compute the magic square of the specified rank, using
                % MATLAB's magic function.
                square = magic(rank);
