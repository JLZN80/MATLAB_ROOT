...
% Use Bloomberg(R) to retrieve raw tick data by specifying a time range for
% each day in a specific date range. Specify the time interval and the
% field for the type of tick data to return. Here, specify the bid tick
% data.
...
%%
% Retrieve the tick series for the |'F US Equity'| security for the last
% two days. Use the time range from the beginning of the trading day
% through noon. Retrieve tick data aggregated into 5-minute intervals.
% Specify retrieving the bid tick series. |d| is a numeric matrix.
